...
const membersControllerPointsAdd = async (req: Request, res: Response) => {
    try {
        ...
        if (points <= 0) throw new InvalidArgumentError('Solo se pueden sumar puntos positivos.');

        const members = mongoDb.collection('members');
        const member = await members.findOneAndUpdate({ _id: new UUID(_id) as any }, { $inc: { points } });
...
export default membersControllerPointsAdd;